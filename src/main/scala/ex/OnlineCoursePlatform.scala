...
object OnlineCoursePlatform:
  ...
  private case class OnlineCoursePlatformImpl() extends OnlineCoursePlatform:
    var courses: Sequence[Course] = empty
    var enrollments: Sequence[(String, String)] = empty
    override def addCourse(course: Course): Unit = if !courses.contains(course) then courses = Cons(course, courses)
    ...
    override def isCourseAvailable(courseId: String): Boolean = courses.filter(c => c.courseId == courseId) != empty
    ...
    override def unenrollStudent(studentId: String, courseId: String): Unit =
      enrollments = enrollments.filter((s,c) => (s,c) != (studentId, courseId))
    override def getStudentEnrollments(studentId: String): Sequence[Course] =
      enrollments.filter((s,c) => s == studentId).map((s,c) => c).map(c => getCourse(c).orElse(Course.createEmpty()))
